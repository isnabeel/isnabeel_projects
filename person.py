...
# -*- coding: utf-8 -*-
"""
Created on Wed Dec 13 07:26:19 2017
...
class Person(object):
    ...
    def add_child(self, child):
        ...
        """
        if self.gender not in [Gender.MALE, Gender.FEMALE]:
            raise PersonError("person's gender '{}' is not known. cannot add child. with unknown gender".format(self.name))
        self.children.add(child)
        
        if self.gender in Gender.MALE:
    ...
    def remove_father(self):
        ...
        if self not in self.father.children:
            raise Exception("'{}'is not a child of father '{}'".format(self.name, self.father.name))
    ...
    def remove_mother(self):        
        ...
        if self not in self.mother.children:
            raise Exception("'{}' is not a child of mother '{}'".format(self.name,self.father.name))
